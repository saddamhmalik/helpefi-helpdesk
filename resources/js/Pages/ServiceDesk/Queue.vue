...
const selectClass = 'w-full rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-sm text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20';
...
<template>
    ...
    <AgentLayout>
        ...
        <DataTable>
            <template #head>
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $t('service_desk.ticket') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $t('service_desk.customer') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $t('service_desk.status') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $t('service_desk.priority') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $t('service_desk.assignee') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $t('service_desk.updated') }}</th>
                </tr>
            </template>
            <template #body>
                ...
                <tr v-for="ticket in tickets.data" :key="ticket.id" class="border-t border-slate-100 hover:bg-slate-50/80">
                    <td class="px-4 py-3">
                        <Link :href="`/tickets/${ticket.id}`" class="group inline-flex items-center gap-2">
                            <span class="font-medium text-slate-900 group-hover:text-blue-600">{{ ticket.subject }}</span>
                            <UnreadBadge v-if="ticket.unread_count" :count="ticket.unread_count" />
                        </Link>
                        <p class="text-xs text-slate-500">{{ ticket.number }}</p>
                    </td>
                    <td class="px-4 py-3 text-sm text-slate-700">{{ ticket.contact?.name || '—' }}</td>
                    <td class="px-4 py-3">
                        <StatusBadge :label="ticket.status?.name" :color="ticket.status?.color" />
                    </td>
                    <td class="px-4 py-3 text-sm text-slate-700">{{ ticket.priority?.name || '—' }}</td>
                    <td class="px-4 py-3 text-sm text-slate-700">{{ ticket.assignee?.name || 'Unassigned' }}</td>
                    <td class="px-4 py-3 text-sm text-slate-500">{{ ticket.updated_at }}</td>
                </tr>
            </template>
        </DataTable>
