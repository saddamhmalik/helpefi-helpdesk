...
<template>
    ...
    <PortalLayout>
        <div class="mb-10 text-center">
            <h1 class="text-3xl font-bold text-slate-900">{{ $t('portal.how_can_we_help') }}</h1>
            <p class="mt-2 text-slate-600">{{ $t('portal.browse_articles_request_services_or_submit_a_support_ticket') }}</p>
            <div class="mx-auto mt-6 flex max-w-xl flex-wrap justify-center gap-2">
                <Link :href="portalPath('/services')" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">{{ $t('portal.browse_services') }}</Link>
            ...
            <form :action="portalPath('/search')" method="get" class="mx-auto mt-4 flex max-w-xl gap-2">
                <input name="q" type="search" :placeholder="$t('portal.search_articles')" class="flex-1 rounded-lg border border-slate-300 px-4 py-2" />
        ...
        <section v-if="collections?.length" class="mb-10">
            <h2 class="mb-4 text-xl font-semibold text-slate-900">{{ $t('portal.collections') }}</h2>
        ...
        <section v-if="featured?.length">
            <h2 class="mb-4 text-xl font-semibold text-slate-900">{{ $t('portal.popular_articles') }}</h2>
            <div class="space-y-3">
                <Link
