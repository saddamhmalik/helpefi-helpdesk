...
<template>
    ...
    <PortalLayout>
        <div class="mb-6">
            ...
            <h1 class="mt-2 text-2xl font-bold text-slate-900">{{ $t('portal.search') }}</h1>
        </div>

        <form :action="portalPath('/search')" method="get" class="mb-6 flex gap-2">
            <input name="q" type="search" :value="query || ''" :placeholder="$t('portal.search_articles')" class="flex-1 rounded-lg border border-slate-300 px-4 py-2" />
