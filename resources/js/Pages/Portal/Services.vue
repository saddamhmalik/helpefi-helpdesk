...
<template>
    ...
    <PortalLayout>
        <div>
            ...
            <h1 class="mt-2 text-2xl font-bold text-slate-900">{{ $t('portal.service_catalog') }}</h1>
            <p class="mt-1 text-sm text-slate-600">{{ $t('portal.choose_a_service_to_submit_a_structured_request') }}</p>
            ...
            <div class="mt-8 space-y-8">
                ...
                <p v-if="!categories?.length" class="text-sm text-slate-500">{{ $t('portal.no_services_are_available_right_now') }}</p>
            </div>
        </div>
    </PortalLayout>
</template>
