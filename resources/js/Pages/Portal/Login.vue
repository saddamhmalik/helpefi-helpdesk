...
<template>
    ...
    <PortalLayout>
        <div class="mx-auto max-w-md">
            ...
            <h1 class="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">{{ $t('portal.sign_in') }}</h1>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">{{ $t('portal.view_and_track_your_support_requests') }}</p>
            ...
            <p class="mt-4 text-center text-sm text-slate-600 dark:text-slate-400">
                {{ $t('portal.no_account') }}
                <Link :href="portalPath('/register')" class="text-blue-600 hover:text-blue-700 dark:hover:text-blue-300 dark:text-blue-300">{{ $t('portal.register') }}</Link>
                {{ $t('portal.or') }}
                <Link :href="portalPath('/track')" class="text-blue-600 hover:text-blue-700 dark:hover:text-blue-300 dark:text-blue-300">{{ $t('portal.track_without_signing_in') }}</Link>
            </p>
        </div>
    </PortalLayout>
</template>
