...
<template>
    <Head :title="`Start free trial · ${brand}`" />
                ...
                <aside class="relative hidden overflow-hidden bg-slate-950 px-10 py-12 text-white lg:flex lg:flex-col lg:justify-between">
                    ...
                    <div class="relative">
                        ...
                        <p class="mt-4 max-w-md text-sm leading-relaxed text-slate-400">
                            Get full access during your trial. When it ends, pick a plan that fits your team — no commitment until then.
                        </p>
                    </div>
                    <ul class="relative space-y-4">
                        <li v-for="item in ['Full platform access during trial', 'Guided email & channel setup', 'Choose a plan only when trial ends']" :key="item" class="flex items-center gap-3 text-sm text-slate-300">
                ...
                <div class="px-4 py-8 sm:px-6 lg:py-12">
                    <div class="mx-auto max-w-lg">
                        <div class="mb-6 lg:hidden">
                            ...
                            <h1 class="mt-4 text-2xl font-semibold text-slate-900">Start your free trial</h1>
                        ...
                        <form class="space-y-6" @submit.prevent="submit">
                            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                                <div class="mb-5 flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 class="font-semibold text-slate-900">Workspace</h2>
                                        <p class="text-xs text-slate-500">Name and URL for your team</p>
                                    </div>
                                </div>
                                <div class="space-y-4">
                                    <div>
                                        <label class="mb-1.5 block text-sm font-medium text-slate-700">Organization name</label>
                                        <input v-model="form.organization_name" type="text" required :class="inputClass" placeholder="Acme Support" />
                                        <p v-if="form.errors.organization_name" class="mt-1.5 text-xs text-red-600">{{ form.errors.organization_name }}</p>
                                    </div>
                                    <div>
                                        <label class="mb-1.5 block text-sm font-medium text-slate-700">Workspace URL</label>
                                        <div class="flex overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20">
                                            <input v-model="form.slug" type="text" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" class="min-w-0 flex-1 border-0 bg-transparent px-3.5 py-2.5 text-sm focus:outline-none" placeholder="acme" @input="onSlugInput" />
                            ...
                            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                                <div class="mb-5 flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 class="font-semibold text-slate-900">Your account</h2>
                                        <p class="text-xs text-slate-500">You'll be the workspace admin</p>
                                    </div>
                                </div>
                                <div class="space-y-4">
                                    <div>
                                        <label class="mb-1.5 block text-sm font-medium text-slate-700">Full name</label>
                                        <input v-model="form.name" type="text" required :class="inputClass" placeholder="Jane Admin" />
                                    </div>
                                    <div>
                                        <label class="mb-1.5 block text-sm font-medium text-slate-700">Work email</label>
                                    ...
                                    <div>
                                        ...
                                        <input v-model="form.password" type="password" required minlength="8" :class="inputClass" placeholder="At least 8 characters" />
                            ...
                            <p class="text-center text-xs text-slate-500">
                                ...
                                <Link href="/login" class="font-medium text-blue-600 hover:text-blue-700">Sign in</Link>
                            </p>
                        </form>
                    </div>
                </div>
            ...
            <Transition name="provision-fade">
                <div v-if="form.processing" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-sm">
                    <div class="provision-card w-full max-w-md rounded-2xl border border-white/10 bg-white p-8 text-center shadow-2xl">
                        ...
                        <p class="mt-6 text-xs text-slate-400">Redirecting to {{ form.slug ? `${form.slug}.${centralDomain}` : 'your workspace' }}…</p>
                    </div>
                </div>
            </Transition>
        </Teleport>
