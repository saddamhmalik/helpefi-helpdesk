...
const props = defineProps({
    ...
    compareHub: { type: Object, default: () => ({}) },
...
const accentMap = {
    ...
    front: 'border-violet-500/20 bg-violet-500/5 hover:border-violet-400/40',
    'help-scout': 'border-amber-500/20 bg-amber-500/5 hover:border-amber-400/40',
    'zoho-desk': 'border-rose-500/20 bg-rose-500/5 hover:border-rose-400/40',
    freshservice: 'border-indigo-500/20 bg-indigo-500/5 hover:border-indigo-400/40',
    'jira-service-management': 'border-blue-500/20 bg-blue-500/5 hover:border-blue-400/40',
};
    ...
    <CentralLayout :brand="brand" :trial-days="trialDays" :social-links="socialLinks">
        <section class="relative overflow-hidden bg-slate-950 py-16 text-white sm:py-24">
            <div class="pointer-events-none absolute inset-0">
                <div class="absolute -left-32 top-0 h-[28rem] w-[28rem] rounded-full bg-teal-600/20 blur-3xl" />
                <div class="absolute -right-32 bottom-0 h-[24rem] w-[24rem] rounded-full bg-blue-600/15 blur-3xl" />
            </div>
            <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <CentralBreadcrumbs />
                <p class="text-sm font-semibold uppercase tracking-wider text-teal-300">{{ compareHub.badge }}</p>
                <h1 class="mt-4 max-w-3xl text-3xl font-extrabold tracking-tight sm:text-5xl">
                    {{ compareHub.hero_title }}
                    <span class="bg-gradient-to-r from-teal-400 via-blue-300 to-cyan-300 bg-clip-text text-transparent">{{ compareHub.hero_highlight }}</span>
                </h1>
                <p class="mt-6 max-w-2xl text-lg text-slate-300">{{ compareHub.hero_subtitle }}</p>
        ...
        <section class="bg-white py-16 dark:bg-slate-900 sm:py-20">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    <Link
                        v-for="compare in comparePages"
                        :key="compare.slug"
                        :href="compare.path"
                        class="group rounded-2xl border p-6 transition dark:border-slate-800"
                        :class="cardClass(compare.slug)"
                    >
                        <p class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                            {{ compare.footer_label ?? `vs ${compare.competitor_name}` }}
                        </p>
        ...
        <section class="border-t border-slate-200 bg-slate-50 py-16 dark:border-slate-800 dark:bg-slate-950 sm:py-20">
            <div class="mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8">
                <h2 class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ compareHub.cta_title }}</h2>
                <p class="mt-4 text-slate-600 dark:text-slate-400">{{ compareHub.cta_body }}</p>
                <Link
                    href="/register"
