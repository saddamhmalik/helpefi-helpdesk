            ...
            <div class="mx-auto grid max-w-4xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:grid-cols-5">
                <aside class="hidden bg-slate-950 p-8 text-white lg:col-span-2 lg:flex lg:flex-col lg:justify-between">
                    ...
                    <div class="rounded-xl border border-white/10 bg-white/5 p-4">
                        <p class="text-xs font-medium text-slate-300">New to {{ brand }}?</p>
                        <p class="mt-1 text-sm text-white">Start a {{ trialDays }}-day free trial — no credit card required.</p>
                ...
                <div class="p-8 lg:col-span-3">
                    <Link href="/" class="text-sm text-slate-500 hover:text-slate-700 lg:hidden">← Back to home</Link>
                    <h1 class="mt-4 text-2xl font-semibold text-slate-900 lg:mt-0">Sign in to your workspace</h1>
                    <p class="mt-2 text-sm text-slate-600">
                        We'll send you to your team's login page.
                    </p>
                        ...
                        <div>
                            ...
                            <div class="flex overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20">
                                <input
                                    ...
                                    autofocus
                                />
                                <span class="flex items-center bg-slate-50 px-3 text-sm text-slate-500">.{{ centralDomain || 'helpdesk.test' }}</span>
                            </div>
                            <p v-if="form.errors.slug" class="mt-1.5 text-xs text-red-600">{{ form.errors.slug }}</p>
                        </div>

                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-slate-700">Email <span class="font-normal text-slate-400">(optional)</span></label>
                            <input v-model="form.email" type="email" :class="inputClass" placeholder="you@company.com" />
                            <p class="mt-1.5 text-xs text-slate-500">Prefills the sign-in form on your workspace.</p>
                        </div>
                    ...
                    <p class="mt-6 text-center text-sm text-slate-600 lg:hidden">
                        No workspace yet?
                        <Link href="/register" class="font-medium text-blue-600 hover:text-blue-700">Start free trial</Link>
                    </p>
                </div>
            </div>
        </div>
