...
import { useMarketingEnglish } from '../../composables/useMarketingEnglish.js';
        ...
        <div class="min-h-[calc(100dvh-3.5rem)] bg-slate-50 dark:bg-slate-950 px-4 py-8 sm:min-h-[calc(100vh-4rem)] sm:px-6 sm:py-12">
            <div class="mx-auto grid max-w-4xl overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm lg:grid-cols-5">
                <aside class="hidden bg-slate-950 p-8 text-white lg:col-span-2 lg:flex lg:flex-col lg:justify-between">
                    <div>
                        <h2 class="text-xl font-semibold">{{ label('welcome_back') }}</h2>
                ...
                <div class="p-6 sm:p-8 lg:col-span-3">
                    <Link href="/" class="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 dark:text-slate-300 lg:hidden">{{ label('back_to_home') }}</Link>
                    <h1 class="mt-4 text-2xl font-semibold text-slate-900 dark:text-slate-100 lg:mt-0">{{ label('sign_in_to_your_workspace') }}</h1>
                    <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">
                        {{ label('well_send_you_to_your_teams_login_page') }}
                    </p>
                    ...
                    <form class="mt-8 space-y-5" @submit.prevent="submit">
                        ...
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{{ label('email') }} <span class="font-normal text-slate-400 dark:text-slate-500">{{ label('optional') }}</span></label>
                            <input v-model="form.email" type="email" :class="inputClass" :placeholder="label('you_company_com')" />
                        ...
                        >{{ label('continue_to_sign_in') }}</button>
                    </form>

                    <p class="mt-6 text-center text-sm text-slate-600 dark:text-slate-400 lg:hidden">
                        {{ label('no_workspace_yet') }}
                        <Link href="/register" class="font-medium text-blue-600 hover:text-blue-700 dark:hover:text-blue-300 dark:text-blue-300">{{ label('start_your_free_trial') }}</Link>
                    </p>
                </div>
            </div>
        </div>
