...
<template>
    ...
    <AdminLayout>
        <div class="mx-auto max-w-5xl px-4 py-8 sm:px-6">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div v-for="group in catalog" :key="group.group" class="rounded-xl border border-slate-200 p-4">
                                ...
                                <div class="mt-3 space-y-2">
                                    <label v-for="permission in group.permissions" :key="permission.name" class="flex items-center gap-2 text-sm text-slate-700">
                                        <input v-model="createForm.permissions" type="checkbox" :value="permission.name" class="rounded border-slate-300 text-blue-600" />
                                        {{ permission.label }}
                                    </label>
                                </div>
                            </div>
            ...
            <div class="space-y-4">
                <div v-for="role in roles" :key="role.id" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <form @submit.prevent="updateRole(role)">
                        ...
                        <div class="mt-5 grid gap-4 sm:grid-cols-2">
                            <div v-for="group in catalog" :key="`${role.id}-${group.group}`" class="rounded-xl border border-slate-200 p-4">
                                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ group.group }}</p>
                                <div class="mt-3 space-y-2">
                                    <label v-for="permission in group.permissions" :key="permission.name" class="flex items-center gap-2 text-sm text-slate-700">
                                        <input
                                            ...
                                            :disabled="!canManage"
                                            class="rounded border-slate-300 text-blue-600 disabled:opacity-60"
                                        />
                                        {{ permission.label }}
                                    </label>
                                </div>
