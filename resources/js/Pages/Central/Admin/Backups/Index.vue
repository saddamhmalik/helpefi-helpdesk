...
<template>
    ...
    <AdminLayout>
        ...
        <div
            ...
            class="mb-6 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-5 py-4 text-sm text-slate-600 dark:text-slate-400 shadow-sm"
        >
            {{ schedule.summary }}
        </div>

        <div v-if="can('backups.manage')" class="mb-6 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $t('central.automatic_backups') }}</h2>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                        {{ $t('central.queue_central_and_workspace_database_snapshots_on_a_schedule_expired_b') }}
                    </p>
                    <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">{{ schedule?.summary }}</p>
                </div>
            ...
            <form class="mt-4 grid gap-4 md:grid-cols-4" @submit.prevent="saveSchedule">
                <div>
                    <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $t('central.frequency') }}</label>
                ...
                <div v-if="scheduleForm.frequency === 'weekly'">
                    <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $t('central.day') }}</label>
                    <select
                ...
                <div>
                    <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $t('central.time') }}</label>
                    <input
        ...
        <div v-if="can('backups.manage')" class="mb-6 grid gap-4 lg:grid-cols-3">
            <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm">
                <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $t('central.central_platform') }}</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $t('central.backup_tenants_subscriptions_payments_and_platform_settings') }}</p>
                <button
                    type="button"
            ...
            <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm">
                <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $t('central.all_workspaces') }}</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $t('central.queue_a_database_snapshot_for_every_customer_workspace') }}</p>
                <button
                    type="button"
                    class="mt-4 rounded-lg border border-slate-300 dark:border-slate-700 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-950 dark:hover:bg-slate-800 disabled:opacity-60"
            ...
            <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm">
                <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $t('central.single_workspace') }}</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $t('central.backup_tickets_customers_and_settings_for_one_workspace') }}</p>
                ...
                <button
                    type="button"
                    class="mt-3 rounded-lg border border-slate-300 dark:border-slate-700 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-950 dark:hover:bg-slate-800 disabled:opacity-60"
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        <tr v-for="backup in backups.data" :key="backup.id">
                            <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ formatDateTime(backup.created_at) }}</td>
                            <td class="px-4 py-3 text-slate-900 dark:text-slate-100">{{ scopeLabel(backup) }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ formatSize(backup.size_bytes) }}</td>
                            <td class="px-4 py-3 text-slate-600 dark:text-slate-400">{{ backup.creator?.name ?? backup.creator?.email ?? '—' }}</td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    <a
                                        v-if="backup.status === 'completed'"
                                        :href="`/admin/backups/${backup.id}/download`"
                                        class="rounded-lg border border-slate-300 dark:border-slate-700 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-950 dark:hover:bg-slate-800"
                                    >
                                        {{ $t('central.download') }}
                                    </a>
                                    <AppDeleteAction
                        ...
                        <tr v-if="!backups.data?.length">
                            <td colspan="6" class="px-4 py-12 text-center text-slate-500 dark:text-slate-400">No backups yet.</td>
                        </tr>
                    </tbody>
                </table>
            </div>

            <div v-if="backups.links?.length > 3" class="border-t border-slate-100 dark:border-slate-800 px-4 py-3">
                <PaginationLinks
                    :links="backups.links"
                    :from="backups.from"
                    :to="backups.to"
