...
const typeStyles = {
    ...
    announcement: 'border-blue-200 bg-blue-50 text-blue-900',
...
<template>
    ...
    <AdminLayout>
        <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6">
            <div class="mb-8">
                <Link href="/admin/notices" class="text-sm text-slate-500 hover:text-slate-700">← Back to notices</Link>
                <h1 class="mt-2 text-2xl font-semibold text-slate-900">{{ isEditing ? 'Edit notice' : 'New notice' }}</h1>
                <p class="mt-1 text-sm text-slate-600">{{ $t('central.create_a_draft_then_publish_when_ready_published_notices_appear_as_ban') }}</p>
            </div>

            <form class="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submit">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('common.title') }}</label>
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('central.type') }}</label>
                        <select v-model="form.notice_type" :class="adminInputClass">
                            <option v-for="(label, value) in types" :key="value" :value="value">{{ label }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('central.priority') }}</label>
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('central.audience') }}</label>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('central.message_html_supported') }}</label>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('central.image_optional') }}</label>
                    <div v-if="imagePreview" class="mb-3 flex items-start gap-3">
                        <img :src="imagePreview" alt="Preview" class="max-h-32 rounded-xl border border-slate-200 object-contain" />
                        <button type="button" class="text-sm text-red-600 hover:text-red-700" @click="removeImage">{{ $t('central.remove_image') }}</button>
                    </div>
                    <input type="file" accept="image/*" class="block w-full text-sm text-slate-600" @change="onImageChange" />
                    <p v-if="form.errors.image" class="mt-1 text-xs text-red-600">{{ form.errors.image }}</p>
                </div>

                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('central.start_optional') }}</label>
                        <input v-model="form.starts_at" type="datetime-local" :class="adminInputClass" />
                    </div>
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('central.end_optional') }}</label>
                        <input v-model="form.ends_at" type="datetime-local" :class="adminInputClass" />
                    </div>
                </div>

                <div>
                    <label class="mb-1.5 block text-sm font-medium text-slate-700">{{ $t('central.target_workspaces') }}</label>
                    <div class="flex flex-wrap gap-2">
                        <label
                            ...
                            :class="form.target_scope === value ? 'border-blue-500 bg-blue-50 text-blue-900' : 'border-slate-200 text-slate-700'"
                        >
                ...
                <div v-if="form.target_scope === 'selected'" class="rounded-xl border border-slate-200 p-4">
                    <div class="mb-3 flex items-center justify-between gap-3">
                        <p class="text-sm font-medium text-slate-700">Selected workspaces ({{ form.tenant_ids.length }})</p>
                        <input v-model="tenantSearch" type="search" :placeholder="$t('central.search_workspaces')" class="w-56 rounded-lg border border-slate-200 px-3 py-1.5 text-sm" />
                    </div>
                    ...
                    <div class="max-h-56 space-y-1 overflow-y-auto">
                        <label
                            ...
                            :key="tenant.id"
                            class="flex cursor-pointer items-center gap-3 rounded-lg px-2 py-2 hover:bg-slate-50"
                        >
                            <input
                                type="checkbox"
                            ...
                            <span class="min-w-0 flex-1">
                                <span class="block text-sm font-medium text-slate-900">{{ tenant.name }}</span>
                ...
                <label class="flex items-center gap-2 text-sm text-slate-700">
                    <input v-model="form.dismissible" type="checkbox" class="rounded border-slate-300" />
                ...
                <div class="flex items-center gap-3">
                    ...
                    <Link href="/admin/notices" class="text-sm text-slate-500 hover:text-slate-700">{{ $t('common.cancel') }}</Link>
                </div>
            </form>
        </div>
    </AdminLayout>
