        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <div class="lg:col-span-2">
                <article class="rounded-xl border agent-border agent-panel p-8 shadow-sm">
                    ...
                    <h1 class="text-3xl font-bold tracking-tight agent-text">{{ article.title }}</h1>

                    <dl class="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                        <div v-if="article.collection?.name">
                            <dt class="font-medium agent-text-subtle">{{ $t('knowledge.collection') }}</dt>
                            <dd class="mt-0.5 agent-text">{{ article.collection.name }}</dd>
                        </div>
                        <div v-if="article.category?.name">
                            <dt class="font-medium agent-text-subtle">{{ $t('knowledge.category') }}</dt>
                            <dd class="mt-0.5 agent-text">{{ article.category.name }}</dd>
                        </div>
                    </dl>

                    <p v-if="article.excerpt" class="mt-6 text-lg leading-relaxed agent-text-muted">{{ article.excerpt }}</p>

                    <div class="mt-8 border-t agent-border-subtle pt-8">
            ...
            <div class="space-y-6">
                <div class="agent-card">
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <Link :href="`/knowledge/${article.id}/edit`" class="text-sm font-medium text-blue-600 hover:text-blue-700 dark:hover:text-blue-300 dark:text-blue-300">{{ $t('knowledge.edit') }}</Link>
                    </div>
