    ...
    >
        ...
        <div v-show="activeSection === 'overview'" class="space-y-6">
                    <div class="grid gap-4 sm:grid-cols-3">
                        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                            <p class="text-sm text-slate-500">{{ $t('settings_security.mfa_adoption') }}</p>
                        ...
                        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                            <p class="text-sm text-slate-500">{{ $t('settings_security.audit_retention') }}</p>
                            <p class="mt-1 text-2xl font-semibold text-slate-900">{{ observability.settings.audit_retention_days }} days</p>
                        </div>
                        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                            <p class="text-sm text-slate-500">{{ $t('settings_security.closed_ticket_retention') }}</p>
                    ...
                    <div class="max-w-2xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                        <h2 class="text-lg font-medium text-slate-900">{{ $t('settings_security.7-day_event_summary') }}</h2>
                        <ul class="mt-4 space-y-2">
                            ...
                            <li v-if="!Object.keys(observability.audit_summary).length" class="text-sm text-slate-500">{{ $t('settings_security.no_audit_events_yet') }}</li>
                        </ul>
                    </div>
                </div>

        <div v-show="activeSection === 'policy'" class="max-w-2xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-lg font-medium text-slate-900">{{ $t('settings.security_policy') }}</h2>
                    <form class="mt-4 space-y-4" @submit.prevent="save">
                        ...
                        <div>
                            <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.audit_log_retention_days') }}</label>
                        ...
                        <div>
                            <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.closed_ticket_retention_days') }}</label>
                            <input v-model="form.closed_ticket_retention_days" type="number" min="30" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" :placeholder="$t('settings_security.leave_blank_to_disable')" />
                        </div>
                        <div class="flex flex-wrap gap-3">
                            ...
                            <button type="button" class="rounded-lg border border-red-200 px-4 py-2 text-sm font-medium text-red-700 hover:bg-red-50" @click="purgeRetention">{{ $t('settings_security.run_retention_purge') }}</button>
                        </div>
                    </form>
                </div>

        <div v-show="activeSection === 'sso'" class="max-w-2xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="text-lg font-medium text-slate-900">{{ $t('settings.single_sign_on') }}</h2>
            <PlanFeatureBanner feature="sso" class="!mb-4" />
            <form class="mt-4 space-y-4" @submit.prevent="saveSso">
                <label class="flex items-center gap-2 text-sm text-slate-700">
                    <input v-model="ssoForm.sso_enabled" type="checkbox" class="rounded border-slate-300" />
                    Enable SSO for agents
                </label>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.protocol') }}</label>
                    <select v-model="ssoForm.sso_protocol" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
                ...
                <template v-if="ssoForm.sso_protocol === 'oidc'">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.provider_preset') }}</label>
                        <select v-model="ssoForm.sso_config.preset" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.client_id') }}</label>
                        <input v-model="ssoForm.sso_config.client_id" type="text" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.client_secret') }}</label>
                        <input v-model="ssoForm.sso_config.client_secret" type="password" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" :placeholder="sso?.sso_config?.has_client_secret ? 'Leave blank to keep current secret' : ''" />
                    </div>
                    <div v-if="ssoForm.sso_config.preset === 'azure'">
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.tenant_id') }}</label>
                        <input v-model="ssoForm.sso_config.tenant_id" type="text" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                    </div>
                    <div v-if="ssoForm.sso_config.preset === 'oidc'">
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.issuer_url') }}</label>
                        <input v-model="ssoForm.sso_config.issuer" type="url" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                    </div>
                </template>
                <template v-else>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.idp_entity_id') }}</label>
                        <input v-model="ssoForm.sso_config.idp_entity_id" type="text" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.sso_url') }}</label>
                        <input v-model="ssoForm.sso_config.sso_url" type="url" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.idp_x509_certificate') }}</label>
                        <textarea v-model="ssoForm.sso_config.x509_cert" rows="4" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-mono" />
                    </div>
                </template>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('settings_security.allowed_email_domains_one_per_line') }}</label>
                    <textarea v-model="ssoForm.sso_config.allowed_domains" rows="3" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="company.com" />
                </div>
        ...
        <div v-show="activeSection === 'audit'" class="max-w-2xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h2 class="text-lg font-medium text-slate-900">{{ $t('settings_security.audit_trail') }}</h2>
                            <p class="mt-1 text-sm text-slate-600">{{ $t('settings_security.full_activity_history_is_available_on_the_audit_logs_page') }}</p>
                        </div>
