...
<template>
    <SettingsPage :title="member.name" :description="member.email" :head-title="`${member.name} · Profile`" info-section="agents" hide-page-header>
        <div class="relative min-w-0 pb-8">
            ...
            <div class="relative mb-8">
                ...
                <div class="mt-4 flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div class="rounded-2xl border agent-border agent-panel px-5 py-4 text-center shadow-sm">
            ...
            <div class="relative grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div class="rounded-2xl border agent-border agent-panel border-l-4 border-l-blue-500 p-5 shadow-sm">
                ...
                <div class="rounded-2xl border agent-border agent-panel border-l-4 border-l-violet-500 p-5 shadow-sm">
                ...
                <div class="rounded-2xl border agent-border agent-panel border-l-4 border-l-cyan-500 p-5 shadow-sm">
                ...
                <div class="rounded-2xl border agent-border agent-panel border-l-4 border-l-amber-500 p-5 shadow-sm">
            ...
            <div class="relative mt-6 grid min-w-0 gap-6 xl:grid-cols-12">
                <div class="min-w-0 space-y-6 xl:col-span-5">
                    <section v-if="allSkills.length" class="rounded-2xl border agent-border agent-panel p-6 shadow-sm">
                        ...
                        <form class="mt-4" @submit.prevent="saveSkills">
                            ...
                            <button type="submit" class="mt-4 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50" :disabled="skillsForm.processing">{{ $t('settings_members_show.save_skills') }}</button>
                        </form>
                    </section>

                    <section v-if="teams.length" class="rounded-2xl border agent-border agent-panel p-6 shadow-sm">
                        ...
                        <form class="mt-4" @submit.prevent="saveTeams">
                            ...
                            <button type="submit" class="mt-4 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50" :disabled="teamsForm.processing">{{ $t('settings_members_show.save_teams') }}</button>
                        </form>
                    </section>

                    <section class="rounded-2xl border agent-border agent-panel p-6 shadow-sm">
                        ...
                        <div v-if="!departments.length" class="mt-4 rounded-xl border border-dashed agent-border px-4 py-8 text-center text-sm agent-text-subtle">
                        ...
                        <div v-else class="mt-4 space-y-4">
                            <div v-for="department in departments" :key="department.id" class="rounded-xl border agent-border-subtle agent-panel-muted/60 p-4">
                                ...
                                <ul class="mt-3 space-y-2">
                                    <li v-for="team in department.teams" :key="team.id" class="flex items-center justify-between rounded-lg agent-panel-muted px-3 py-2 text-sm">
                                        <span class="font-medium agent-text">{{ team.name }}</span>
                    ...
                    <section v-if="customFieldDefinitions.length && Object.keys(member.custom_fields ?? {}).length" class="rounded-2xl border agent-border agent-panel p-6 shadow-sm">
                ...
                <div class="min-w-0 space-y-6 xl:col-span-7">
                    <section class="rounded-2xl border agent-border agent-panel p-6 shadow-sm">
                        ...
                        <div class="mt-4 grid gap-3 sm:grid-cols-2">
                            <div>
                                <p class="mb-2 text-xs font-semibold uppercase tracking-wide agent-text-subtle">{{ $t('settings_members_show.by_status') }}</p>
                                <div class="space-y-2">
                                    ...
                                    <p v-if="!assignedByStatus.length" class="text-sm agent-text-subtle">{{ $t('settings_members_show.no_assigned_tickets') }}</p>
                                </div>
                            </div>
                            <div>
                                <p class="mb-2 text-xs font-semibold uppercase tracking-wide agent-text-subtle">{{ $t('settings_members_show.open_by_priority') }}</p>
                                <div class="space-y-2">
                                    ...
                                    <p v-if="!assignedByPriority.length" class="text-sm agent-text-subtle">{{ $t('settings_members_show.no_open_assigned_tickets') }}</p>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="rounded-2xl border agent-border agent-panel p-6 shadow-sm">
                        ...
                        <ul class="mt-4 divide-y agent-table-divider">
                            <li v-for="event in recentPerformanceEvents.data" :key="event.id" class="flex items-center justify-between gap-4 py-3 first:pt-0">
                                <div>
                                    <p class="text-sm font-medium capitalize agent-text">{{ event.event_type.replaceAll('_', ' ') }}</p>
                            ...
                            <li v-if="!recentPerformanceEvents.data?.length" class="py-6 text-center text-sm agent-text-subtle">{{ $t('settings_members_show.no_performance_events_yet') }}</li>
                        </ul>
                    </section>
                </div>
            </div>
