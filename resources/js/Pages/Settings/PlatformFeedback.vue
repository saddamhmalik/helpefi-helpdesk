    ...
    >
        <div class="max-w-2xl rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form class="mt-6 space-y-5" @submit.prevent="submit">
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="feedback-type">{{ $t('settings_platform_feedback.type') }}</label>
                    <select
                        id="feedback-type"
                        v-model="form.type"
                        class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
                    >
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="feedback-subject">{{ $t('settings_platform_feedback.subject') }}</label>
                    <input
                        id="feedback-subject"
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="feedback-body">{{ $t('settings_platform_feedback.details') }}</label>
                    <textarea
                        id="feedback-body"
                        v-model="form.body"
                        rows="8"
