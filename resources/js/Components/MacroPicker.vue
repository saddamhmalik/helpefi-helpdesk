...
<template>
    <div ref="panelRef" class="relative">
        <button
            type="button"
            class="inline-flex h-8 items-center gap-1 rounded-lg border border-slate-200 px-2.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50"
            @click.stop="toggle"
        >
        ...
        <div
            v-if="open"
            class="absolute bottom-full left-0 z-30 mb-2 w-80 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl"
            @click.stop
        >
            ...
            <ul class="max-h-64 overflow-y-auto py-1">
                ...
                <li v-for="macro in results" :key="macro.id">
                    ...
                    >
                        <span class="text-sm font-medium text-slate-900">{{ macro.title }}</span>
                        <span v-if="macro.shortcut" class="text-[11px] text-slate-400">#{{ macro.shortcut }}</span>
                    </button>
                </li>
            </ul>
        </div>
