...
const submit = async () => {
    ...
    try {
        ...
        error.value = t('central.marketing_leads.capture_error');
    } finally {
        loading.value = false;
    }
};
        ...
        <div class="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <div class="grid gap-8 lg:grid-cols-2 lg:items-center">
                <div>
                    ...
                    >
                        {{ $t('central.marketing_leads.home_trial_link') }} →
                    </Link>
                </div>
                ...
                >
                    <div v-if="submitted" class="py-4 text-center">
                        <p class="text-lg font-semibold" :class="isDark ? 'text-white' : 'text-slate-900 dark:text-slate-100'">
                            {{ $t('central.marketing_leads.home_success_title') }}
                        </p>
                        <p class="mt-2 text-sm" :class="isDark ? 'text-slate-300' : 'text-slate-600 dark:text-slate-400'">
                            {{ $t('central.marketing_leads.home_success_body') }}
                        </p>
                    </div>
                    ...
                    <form v-else class="space-y-4" @submit.prevent="submit">
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                <label class="mb-1.5 block text-sm font-medium" :class="isDark ? 'text-slate-200' : 'text-slate-700 dark:text-slate-300'">
                                    {{ $t('central.marketing_leads.email_label') }}
                                </label>
                                <input
                                    v-model="email"
                                    type="email"
                            ...
                            <div>
                                <label class="mb-1.5 block text-sm font-medium" :class="isDark ? 'text-slate-200' : 'text-slate-700 dark:text-slate-300'">
                                    {{ $t('central.marketing_leads.name_label') }}
                                </label>
                                <input
                                    v-model="name"
                                    type="text"
                            ...
                            <div>
                                <label class="mb-1.5 block text-sm font-medium" :class="isDark ? 'text-slate-200' : 'text-slate-700 dark:text-slate-300'">
                                    {{ $t('central.marketing_leads.company_label') }}
                                </label>
                                <input
                                    v-model="company"
                                    type="text"
                        ...
                        <label class="flex items-start gap-3 text-sm" :class="isDark ? 'text-slate-300' : 'text-slate-600 dark:text-slate-400'">
                            ...
                            <span>{{ $t('central.marketing_leads.consent_label') }}</span>
                        </label>
                        ...
                        >
                            {{ loading ? $t('central.marketing_leads.submitting') : $t('central.marketing_leads.home_submit') }}
                        </button>
                    </form>
                </div>
            </div>
