...
<template>
    <div
        ...
        :class="[
            ...
            tone === 'dark' ? 'border-white/10 bg-white/5' : variant === 'hero' ? 'border-slate-200/80 bg-slate-50/80' : '',
        ]"
    >
        <div v-if="variant === 'hero'" class="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div>
                <p class="text-sm font-semibold" :class="tone === 'dark' ? 'text-white' : 'text-slate-900'">{{ $t('auth.help_center') }}</p>
                <p class="mt-0.5 text-xs" :class="tone === 'dark' ? 'text-slate-400' : 'text-slate-500'">
            ...
            <Link
                ...
                :class="tone === 'dark' ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'"
            >
                {{ helpCenter.title }} →
            </Link>
        </div>
        ...
        <form :action="helpCenter.searchUrl" method="get" class="flex gap-2" :class="variant === 'hero' ? '' : 'max-w-md'">
            ...
            <button
                ...
                :class="tone === 'dark' ? 'bg-blue-600 hover:bg-blue-500' : 'bg-slate-900 hover:bg-slate-800'"
            >
                {{ $t('auth.search') }}
            </button>
        </form>
