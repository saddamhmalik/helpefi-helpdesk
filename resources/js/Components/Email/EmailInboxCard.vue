    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/80 px-5 py-4">
            ...
            >
                <div class="flex flex-wrap items-center gap-2">
                    <h3 class="font-semibold text-slate-900">{{ localInbox.name }}</h3>
                    <span class="rounded-full px-2.5 py-0.5 text-xs font-medium" :class="methodBadge.class">{{ methodBadge.label }}</span>
                    <span v-if="!localInbox.is_active" class="rounded-full bg-slate-200 px-2.5 py-0.5 text-xs font-medium text-slate-600">{{ $t('components.inactive') }}</span>
                </div>
                <p class="mt-0.5 text-sm text-slate-600">{{ localInbox.address }}</p>
            </button>
        ...
        <AppCollapse :open="isExpanded">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.display_name') }}</label>
                    <input v-model="localInbox.name" type="text" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.email_address') }}</label>
                    <input v-model="localInbox.address" type="email" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-3">
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.brand') }}</label>
                    <select v-model="localInbox.brand_id" required class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                        <option v-for="brand in brandOptions" :key="brand.id" :value="brand.id">{{ brand.name }}</option>
                    </select>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.department') }}</label>
                    <select v-model="localInbox.department_id" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.team') }}</label>
                    <select v-model="localInbox.team_id" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" :disabled="!filteredTeams.length">
                        <option :value="null">{{ $t('components.none') }}</option>
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.aliases') }}</label>
                <textarea v-model="aliasesText" rows="3" :placeholder="$t('components.aliases_placeholder')" class="w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-sm" />
            </div>
            ...
            <div v-show="methodTab === 'webhook'" class="space-y-4 rounded-xl border border-sky-100 bg-sky-50/40 p-5">
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.webhook_url') }}</label>
                    <input :value="localInbox.inbound_webhook_url" type="text" readonly class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 font-mono text-xs text-slate-700" />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.inbound_token') }}</label>
                    <div class="flex gap-2">
                        <input :value="localInbox.inbound_token" type="text" readonly class="min-w-0 flex-1 rounded-lg border border-slate-300 bg-white px-3 py-2 font-mono text-xs text-slate-700" />
                        <button type="button" class="shrink-0 rounded-lg border border-slate-300 px-3 py-2 text-sm" @click="copyToken">{{ tokenCopied ? $t('components.copied') : $t('components.copy') }}</button>
                        <button type="button" class="shrink-0 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900" @click="regenerateToken">{{ $t('components.regenerate') }}</button>
                    </div>
                </div>
                <p class="break-all rounded bg-slate-50 px-2 py-1.5 font-mono text-[11px] text-slate-700">{{ webhookForwardingAddress }}</p>
            </div>

            <div v-show="methodTab === 'poll'" class="space-y-5 rounded-xl border border-violet-100 bg-violet-50/30 p-5">
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.email_provider') }}</label>
                    <select v-model="localInbox.mailbox_provider" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" @change="applyProvider">
                    ...
                    <p v-if="selectedProviderHelp" class="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-900">{{ selectedProviderHelp }}</p>
                </div>
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.protocol') }}</label>
                        <select v-model="localInbox.mailbox_protocol" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                    ...
                    <div>
                        ...
                        <input v-model="localInbox.mailbox_host" type="text" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.port') }}</label>
                        <input v-model.number="localInbox.mailbox_port" type="number" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.encryption') }}</label>
                        <select v-model="localInbox.mailbox_encryption" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
                    ...
                    <div>
                        ...
                        <input v-model="localInbox.mailbox_username" type="text" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700">{{ $t('components.app_password') }}</label>
                        <input v-model="mailboxPassword" type="password" :placeholder="localInbox.has_mailbox_password ? $t('components.app_password_keep') : $t('components.app_password_enter')" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" />
                    </div>
                    <div v-show="localInbox.mailbox_protocol === 'imap'">
                        ...
                        <input v-model="localInbox.mailbox_folder" type="text" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" />
                    </div>
                </div>
                <p v-if="localInbox.poll_error" class="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-800">{{ localInbox.poll_error }}</p>
                <div class="flex flex-wrap items-center gap-3 border-t border-violet-100 pt-4">
                    <button type="button" class="rounded-lg border border-violet-200 bg-white px-4 py-2 text-sm" :disabled="mailboxTesting" @click="testMailbox">{{ mailboxTesting ? $t('components.testing') : $t('components.test_connection') }}</button>
                    <button type="button" class="rounded-lg border border-violet-200 bg-white px-4 py-2 text-sm" @click="pollMailbox">{{ $t('components.check_for_mail_now') }}</button>
                    <span class="text-xs text-slate-500">{{ $t('components.last_checked', { date: formatPolledAt(localInbox.last_polled_at) }) }}</span>
                </div>
            </div>

            <div v-show="methodTab === 'oauth'" class="space-y-4 rounded-xl border border-indigo-100 bg-indigo-50/30 p-5">
                <p class="text-sm font-medium text-indigo-900">{{ $t('components.oauth_mailbox') }}</p>
                <div class="space-y-3">
                    <div v-for="provider in oauthProviderList" :key="provider.key" class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3">
                        <div>
                            <p class="text-sm font-medium text-slate-900">{{ provider.label }}</p>
                            <p v-if="isProviderConnected(provider.key)" class="mt-1 text-xs font-medium text-emerald-700">{{ $t('components.connected_as', { email: localInbox.oauth_connected_email }) }}</p>
                            <p v-else-if="!provider.configured" class="mt-1 text-xs text-amber-700">{{ $t('components.not_configured_on_server') }}</p>
                        </div>
                        <button v-if="isProviderConnected(provider.key)" type="button" class="rounded-lg border border-red-200 px-3 py-1.5 text-sm text-red-700" @click="disconnectOAuth">{{ $t('components.disconnect') }}</button>
            ...
            <div class="flex flex-wrap items-center gap-3 border-t border-slate-100 pt-4">
                <label class="flex items-center gap-2 text-sm text-slate-700">
                    <input v-model="localInbox.is_active" type="checkbox" class="rounded border-slate-300 text-blue-600" />
                ...
                <div class="ml-auto flex flex-wrap gap-2">
                    ...
                    <button type="button" class="rounded-lg border border-red-200 px-4 py-2 text-sm text-red-700 hover:bg-red-50" @click="emit('remove', localInbox)">{{ $t('components.remove') }}</button>
                </div>
            </div>
            </form>
        </AppCollapse>
